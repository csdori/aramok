...
memb.currents<-memb.currents[sorrend,]


############## Source coordinates
M<-as.numeric(read.table('basenum.txt')) #number of sources (a feltételezett források szama a sejten)
delta<-cell.length/(M-7) #shift of overlapping sources on average
#delta<-cell.length/(M) 
source.cord<-array(0,c(M,3))
for (i in 1:M) source.cord[i,3]<-min(seg.kord[,3])+(i-5)*delta
...
where.db<-100
where.cord<-matrix(0,nrow=100,ncol=3)
where.cord[,3]<-seq(seg.end[1,3],seg.end[seg.db,3],length.out=100)
...
par(mfrow=c(2,1))
image.plot(time,seg.kord[,3],t(memb.currents),main='Membrane currents',xlab="time (ms)",ylab="y (um)",zlim=limitsodcurr)
...
#snap RMSE
name.output.tRMSE<-paste('out_0528/tRMSE_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
#write.table(rmse.snap, name.output.tRMSE,col.names=FALSE, row.names=FALSE)
name.output.ntRMSE<-paste('out_0528/normtRMSE_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
#write.table(nrmse.snap, name.output.ntRMSE,col.names=FALSE, row.names=FALSE)
name.output.RMSE.total<-paste('out_0528/RMSEtot_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
#write.table(rmse.total, name.output.RMSE.total,col.names=FALSE, row.names=FALSE)
name.output.nRMSE.total<-paste('out_0528/normRMSEtot_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
write.table(nrmse.total, name.output.nRMSE.total,col.names=FALSE, row.names=FALSE)


name.output.measure1<-paste('out_0528/meaure1_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
write.table(measure1, name.output.measure1,col.names=FALSE, row.names=FALSE)

name.output.measure.curr<-paste('out_0528/measurecurr_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
write.table(measure.curr, name.output.measure.curr,col.names=FALSE, row.names=FALSE)

name.output.measure.peak<-paste('out_0528/measurepeak_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
...
plot(where.cord[,3],C[,min.indexes[2]],col='RED',t='l')
points(seg.kord[,3],memb.currents.vonal[,min.indexes[2]])
lines(seg.kord[,3],memb.currents.vonal.smoothed[,min.indexes[2]])
