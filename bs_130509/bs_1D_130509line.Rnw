...
delta<-cell.length/(M-1) #shift of overlapping sources on average
#delta<-cell.length/(M) 
source.cord<-array(0,c(M,3))
for (i in 1:M) source.cord[i,3]<-min(seg.kord[,3])+(i-2)*delta
...
plot(where.cord[,3],C[,min.indexes[2]],col='RED',t='l')
lines(seg.kord[,3],memb.currents.vonal[,min.indexes[2]],col='RED')
