...
return(out)}
sinxintc<-integrate(sinxx,-R/2,R/2)$value
#if (base=='sinxpx') C.curr<- sinxintc*C #step function eset 
C.curr<-C
for (i in 1: seg.db) C.curr[i,]<-C[i,]*seg.length[i]
C.dens<-C
...
image.plot(time,where.cord[,3],t(C.curr),main='Estimated MC',xlab="time (ms)",ylab="y (um)",zlim=limitsodcurr)
...
<<splineinterpol2,echo=FALSE,fig=TRUE>>=
####################xinterpolation
...
#snap RMSE
name.output.tRMSE<-paste('out0516/tRMSE_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
#write.table(rmse.snap, name.output.tRMSE,col.names=FALSE, row.names=FALSE)
name.output.ntRMSE<-paste('out0516/normtRMSE_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
#write.table(nrmse.snap, name.output.ntRMSE,col.names=FALSE, row.names=FALSE)
name.output.RMSE.total<-paste('out0516/RMSEtot_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
#write.table(rmse.total, name.output.RMSE.total,col.names=FALSE, row.names=FALSE)
name.output.nRMSE.total<-paste('out0516/normRMSEtot_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
write.table(nrmse.total, name.output.nRMSE.total,col.names=FALSE, row.names=FALSE)


name.output.measure1<-paste('out0516/meaure1_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
write.table(measure1, name.output.measure1,col.names=FALSE, row.names=FALSE)

name.output.measure.curr<-paste('out0516/measurecurr_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
write.table(measure.curr, name.output.measure.curr,col.names=FALSE, row.names=FALSE)

name.output.measure.peak<-paste('out0516/measurepeak_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
