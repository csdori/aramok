...
M<-as.numeric(as.matrix(read.table(paste(outname,'/bestparams',sep='')))[6,1])
}
@
...
R<-as.numeric(as.matrix(read.table(paste(outname,'/bestparams',sep=''))[3,1]))

#base<-'step' #type of base function ('step', 'Gaussian')
#base<-'gauss'
base<-as.matrix(read.table(paste(outname,'/bestparams',sep='')))[2,1] #getting it from a text file made by the .sh file
...
C.name<-paste(outname,'/skCSD_current', sep='')
write.table(C,C.name,col.names=FALSE, row.names=FALSE)

tmatr.name<-paste(outname,'/transfermatrix', sep='')
write.table(t(K.tilda)%*%ginv(K),tmatr.name,col.names=FALSE, row.names=FALSE)
...
plot(seg.cord[,1],seg.cord[,3],pch=16,col=szinskala2,asp=1,main='Original')
...
plot(where.cord[,1],where.cord[,3],pch=16,col=szinskala,asp=1,main='skCSD')
legend.col(col=rainbow(150),lev=c(-col.limits,col.limits))
dev.off()
...
%paty yang poland
%Tennivalo:
%ahol a sejten abrazojuk az aramokat a videohoz, ott mellé lehetne rakni a mért LFP-t, a számolt CSD-t és esetleg sCSD-t...
