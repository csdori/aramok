...
#Sweave('/media/BA0ED4600ED416EB/agy/kCSD/progik/bs_futtat/branching/ksCSD_best.Rnw')
...
M<-as.numeric(as.matrix(read.table('out_branch/bestparams'))[6,1])
}
@
...
R<-as.numeric(as.matrix(read.table('out_branch/bestparams'))[3,1])

#base<-'step' #type of base function ('step', 'Gaussian')
#base<-'gauss'
base<-as.matrix(read.table('out_branch/bestparams'))[2,1] #getting it from a text file made by the .sh file
...
name.C.smoothed.abs<-paste('out_branch/smoothed_error_',base,'_bwidth',R, '_dist',d, '_el',el.nb,'_seg', seg.db,'_bnum',M, sep='')
...
<<thingstofile,echo=FALSE>>=

#write the currents to the file
C.name<-paste('out_branch/skCSD_current', sep='')
write.table(C,C.name,col.names=FALSE, row.names=FALSE)

tmatr.name<-paste('out_branch/transfermatrix', sep='')
write.table(t(K.tilda)%*%solve(K),tmatr.name,col.names=FALSE, row.names=FALSE)

for (i in 1: length(time)){
picname<-paste('out_branch/pics/skCSD_C',i,'.png', sep='')
png(picname)
par(mfrow=c(1,2),mar=c(2, 2, 1, 5),bg='BLACK',col.axis='WHITE',col.lab='WHITE',col.main='WHITE',oma=c(0,0,3,0))

min.indexes<-which(C==min(C),arr.ind=TRUE)[2]
#col.limits<-max(abs(range(C[,min.indexes],memb.currents.vonal[,min.indexes])))
col.limits<-max(abs(range(C, memb.currents.vonal)))#,memb.currents.vonal[,min.indexes])))


szinskala2<-color.scale(c(memb.currents.vonal[,i]),col=rainbow(150),zlim=c(-col.limits*1.01,col.limits*1.01))
plot(seg.cord[,1],seg.cord[,2],pch=16,col=szinskala2,asp=1,main='Original')
#text(labels=paste('time: ',time[i]),4,col='WHITE',adj=1,cex=1.5)
mtext(paste(time[i], 'ms'),3,col='WHITE',cex=1.5,outer = TRUE)



szinskala<-color.scale(c(C[,i]),col=rainbow(150),zlim=c(-col.limits*1.01,col.limits*1.01))
plot(where.cord[,1],where.cord[,2],pch=16,col=szinskala,asp=1,main='skCSD')
legend.col(col=rainbow(150),lev=c(-col.limits,col.limits))
dev.off()
