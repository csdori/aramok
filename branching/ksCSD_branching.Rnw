...
#Sweave('/media/BA0ED4600ED416EB/agy/kCSD/progik/bs_futtat/branching/ksCSD_branching.Rnw')
...
xi<-elec.kord
xj<-where.cord
T<-array(0,c(el.nb,source.nb))
for(i in 1:el.nb){
for(j in 1:source.nb){
...
max.limits<-max(abs(range(LFP,LFP.est)))#,memb.currents.vona
szinskala2<-color.scale(c(t(LFP)),col=rainbow(150),zlim=c(-max.limits,max.limits))



par(mfrow=c(2,1), mar=c(2,1,1,5))
image.plot( t(LFP.est), zlim=c(-max.limits,max.limits), nlevel=64,  horizontal=FALSE, col=rainbow(64) , main='Est LFP' )

image.plot(t(LFP), zlim=c(-max.limits,max.limits), nlevel=64, horizontal=FALSE, col=rainbow(64),main='LFP')
...
write.table(name.lfp.error.abs, name.lfp.error.abs,col.names=FALSE, row.names=FALSE)
