...
t<-array(seq(0.05,0.95,length.out=source.branch.db[i]),c(1,source.branch.db[i]))
...
szinskala<-color.scale(c(C[,min.indexes]),col=tim.colors(256))
plot(where.cord[,1],where.cord[,3],pch=16,col=szinskala,asp=1)

szinskala2<-color.scale(c(memb.currents[,min.indexes]),col=tim.colors(256))
plot(seg.cord[,1],seg.cord[,3],pch=16,col=szinskala2)
