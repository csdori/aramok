...
segbranch.num<-read.table(paste(workingdirectory,'/segcoords/branchnum',sep=''))[1,1]
for (i in 1:segbranch.num){ 
segmentname<-paste(workingdirectory,'/segcoords/segcord',i,sep='')
segbranch.coords[[i]]<-matrix(as.matrix(read.table(segmentname)),ncol=3)
}

par(mfrow=c(2,1))
#3D plot  from LFP segments
colours<-rainbow(segbranch.num)
limx<-range(alak[,3],elec.kord[,1])
limy<-range(alak[,4],elec.kord[,2])
limz<-range(alak[,5],elec.kord[,3])
sc<-scatterplot3d(segbranch.coords[[1]][,1],segbranch.coords[[1]][,3],segbranch.coords[[1]][,2],color=colours[1],pch=20, xlim=limx, ylim=limz, zlim=limy,main='The cell and the electrode')
for (i in 1:segbranch.num){ 
sc$points3d(segbranch.coords[[i]][,1],segbranch.coords[[i]][,3],segbranch.coords[[i]][,2],col=colours[i])
}
legend('topright',as.character(c(1:segbranch.num)),col=rainbow(segbranch.num), lty=1,bg='WHITE')
...
########################################################
segbranch.comp.nb<-numeric(segbranch.num)
for (i in 1:segbranch.num) segbranch.comp.nb[i]<-dim(segbranch.coords[[i]])[1]-1

branch.length<-numeric(segbranch.num) #length of branches
comp.length<-list(segbranch.num) #compartments in the branches
comp.place<-list(segbranch.num) #the distance measured from the beginning of the branch

for(j in 1: segbranch.num){
lengthcomp<-numeric()
compplace<-0

for (i in 1: (segbranch.comp.nb[j])){
length<-sqrt(sum((segbranch.coords[[j]][i+1,]-(segbranch.coords[[j]][i,]))^2))
...
vonal<-list(segbranch.num)
vonalfun<-list(segbranch.num)
for(j in 1:segbranch.num){
#soma
...
branch.source<-rep(1:segbranch.num,source.branch.db)
where.branch.db<-source.branch.db*4
...
if (base=='gauss'){
for(j in 1:branch.nb){
for(k in 1:where.branch.db[j]){
x<-matrix((0:branch.length[j]),ncol=1)
gaus<-function(r.t) exp(-sum((where.cord.t[which(where.which==j)[k]]-r.t)^2)/(R^2))
y<-apply(x,1,gaus)
