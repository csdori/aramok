...
@
Let us see the crossvalidation error and the crossvalidated potential
<<crossval,fig=TRUE,echo=FALSE>>=
#####################
##########Crossvalidation
#########################
source('alprogik/ksCSD_crossvalid.R')
cross.valid.error<-cross.valid(LFP,t(K.tilda)%*%ginv(K))[[1]]
fi_star<-cross.valid(LFP,t(K.tilda)%*%ginv(K))[[2]] #crossvalidated potential
zelim<-range(LFP,fi_star)
par(mfrow=c(2,1))
image(t(as.matrix(LFP)),zlim=zelim, main='Original LFP')
image(t(fi_star),zlim=zelim,main='CV LFP')

write.table(cross.valid.error,'cross_valid_error',col.names=FALSE, row.names=FALSE)
@
...
#lets save it also as the actual value with all the other error measures
write.table(current.error.smoothed.abs,'smoothed_error_current',col.names=FALSE, row.names=FALSE)
write.table(current.error.abs,'current_error_current',col.names=FALSE, row.names=FALSE)
write.table(lfp.error.abs,'lfp_error_current',col.names=FALSE, row.names=FALSE)
...
whicherror<-cross.valid.error   #current.error.smoothed.abs
...
if(bestparams[1] >whicherror){
bestparams<-c(whicherror,base,R,el.nb,seg.db,M)
