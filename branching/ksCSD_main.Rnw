...
if (file.exists('out_branch/bestparams')== TRUE ){
bestparams<-as.matrix(read.table('out_branch/bestparams'))
...
write.table(bestparams,'out_branch/bestparams',col.names=FALSE, row.names=FALSE)
}
}

if (file.exists('out_branch/bestparams')== FALSE ){
bestparams<-c(current.error.smoothed.abs,base,R,el.nb,seg.db,M)
write.table(bestparams,'out_branch/bestparams',col.names=FALSE, row.names=FALSE)
}
