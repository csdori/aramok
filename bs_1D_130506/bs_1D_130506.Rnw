...
#A stimulalas is bele megy a sejtbe, az is szami
#memb.currents[1,]<-memb.currents[1,]-stim.currents

#Let's calculate the current density
seg.volume<-seg.length*pi*(seg.diam/2)^2
seg.surface<-seg.length*2*pi*(seg.diam/2)
...
memb.currents.tot<-apply(memb.currents,2,funaramsur)
memb.currents.density<-apply(memb.currents,2,funoszt)
#kiszamolom en az LFP-t
xi<-elec.kord
...
#MOST az altalam hasznalt LFP.t hasznaljuk!!!
...
R<- 30 #width of Gaussian or length of step funtion
delta<-cell.length/M #shift of overlapping sources
...
#most tudom, hogy 500 mikron hosszú a sejt és egyenes
source.cord<-array(0,c(M,3))
#for (i in 1:M) source.cord[i,3]<-(i-1)*delta
source.cord[,3]<-seg.kord[,3]


const<-1/(4*pi*sigma) #reméljük, hogy ők is um-erben mérik a távolságot
...
if (base=='step'){ 
if ((r.t[3] >= (source.cord[i,3])-R/2 ) && (r.t[3] <= (source.cord[i,3])+R/2 )&& (r.t[1]^2+r.t[2]^2)<=R^2)
out<-1
...
if (base=='step'){
if ((r.t[3] >= (source.cord[i,3])-R/2 ) && (r.t[3] <= (source.cord[i,3])+R/2 )&& (r.t[1]^2+r.t[2]^2)<=10^2) ff<- 1/sqrt(sum((r-r.t)^2))
else ff<-0
}
...
<<spatialspike2, echo=FALSE,fig=TRUE>>=
plot(seg.kord[,3],memb.currents[,spikehol],t='l',xlab='y (um)',ylab='current (nA)',main='Comparison of methods at the peak of the spike')
lines(seg.kord[,3],CSD.trad.interpol[,spikehol],col='RED')
lines(seg.kord[,3],C[,spikehol],col='BLUE')
